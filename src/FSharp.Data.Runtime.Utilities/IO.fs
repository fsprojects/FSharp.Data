    ...
    member x.Resolve(uri: Uri) =
        if uri.IsAbsoluteUri then
            ...
            let root =
                ...
                | DesignTime ->
                    if String.IsNullOrEmpty x.ResolutionFolder then
                        x.DefaultResolutionFolder
                    else
                        x.ResolutionFolder
...
let private appendToLog logFile line = appendToLogMultiple logFile [ line ]
...
let internal asyncRead (uriResolver: UriResolver) formatName encodingStr (uri: Uri) =
    ...
    if isWeb then
        async {
            ...
            let! text =
                Http.AsyncRequestString(uri.OriginalString, headers = headers, responseEncodingOverride = encodingStr)
