...
let private getAttributes culture (element:XElement) =
  [ for attr in element.Attributes() do
      yield { Name = attr.Name.ToString()
              Optional = false; 
              Type = StructuralInference.inferPrimitiveType culture attr.Value None } ]
        ...
        let bodyType = 
          [ for e in elements do
              if not (String.IsNullOrEmpty(e.Value)) then
                yield StructuralInference.inferPrimitiveType culture e.Value None ]
  ...
  let props = 
    [ // Generate record fields for attributes
      ...
      if Seq.length children > 0 then
        let collection = StructuralInference.inferCollectionType allowNulls (Seq.map (inferLocalType culture allowNulls) children)
      ...
      // If it has value, add primtiive content
        ...
        let primitive = StructuralInference.inferPrimitiveType culture element.Value None
