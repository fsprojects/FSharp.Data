    ...
    let parse (extension:string) value =
      ...
      let value = 
        if value = "" then 
          ...
          value
      CsvFile.Parse(value, separators, quote, hasHeaders, ignoreErrors, skipRows, ignoreLinePattern)
        ...
        CreateFromTextReader = fun reader ->
          let body = 
            csvErasedType?Create () (Expr.Var stringArrayToRowVar, Expr.Var rowToStringArrayVar, reader, 
                                     separators, quote, hasHeaders, ignoreErrors, skipRows, cacheRows, ignoreLinePattern)
