...
type public WorldBankProvider(cfg: TypeProviderConfig) as this =
        ...
        ProviderHelpers.getOrCreateProvidedType cfg this worldBankTypeName (fun () ->
            ...
            let resTy =
                ProvidedTypeDefinition(
                    ...
                    worldBankTypeName,
                    None,
                    hideObjectMethods = addAttributes,
                    nonNullable = addAttributes
                )

            let serviceTypesType =
                let t =
                    ProvidedTypeDefinition("ServiceTypes", None, hideObjectMethods = true, nonNullable = true)
                          ...
                          let prop =
                              if asynchronous then
                                  ...
                                  ProvidedProperty(
                                      ...
                                      getterCode =
                                          (fun (Singleton arg) ->
                                              <@@ ((%%arg: Indicators) :> IIndicators).GetIndicator(indicatorIdVal) @@>)
                                  )

                          if not (String.IsNullOrEmpty indicator.Description) then
                              prop.AddXmlDoc(indicator.Description)
            ...
            let indicatorsDescriptionsType =
                let t =
                    ProvidedTypeDefinition(
                        ...
                        Some typeof<IndicatorsDescriptions>,
                        hideObjectMethods = true,
                        nonNullable = true
                    )
                          ...
                          let prop =
                              ProvidedProperty(
                                  ...
                                  getterCode =
                                      (fun (Singleton arg) ->
                                          <@@
                                              ((%%arg: IndicatorsDescriptions) :> IIndicatorsDescriptions)
                                                  .GetIndicator(indicatorIdVal)
                                          @@>)
                              )
                ...
                t.AddMembersDelayed(fun () ->
                    [ let prop =
                          ProvidedProperty(
                              ...
                              indicatorsType,
                              getterCode =
                                  (fun (Singleton arg) -> <@@ ((%%arg: Country) :> ICountry).GetIndicators() @@>)
                          )
            ...
            let countriesType =
                let countryCollectionType =
                    ProvidedTypeBuilder.MakeGenericType(typedefof<CountryCollection<_>>, [ countryType ])

                let t =
                    ProvidedTypeDefinition(
                        "Countries",
                        Some countryCollectionType,
                        hideObjectMethods = true,
                        nonNullable = true
                    )
                          ...
                          let prop =
                              ProvidedProperty(
                                  ...
                                  getterCode =
                                      (fun (Singleton arg) ->
                                          <@@
                                              ((%%arg: CountryCollection<Country>) :> ICountryCollection)
                                                  .GetCountry(countryIdVal, name)
                                          @@>)
                              )
            ...
            let regionType =
                let t =
                    ProvidedTypeDefinition("Region", Some typeof<Region>, hideObjectMethods = true, nonNullable = true)
            ...
            let topicType =
                let t =
                    ProvidedTypeDefinition("Topic", Some typeof<Topic>, hideObjectMethods = true, nonNullable = true)

                t.AddMembersDelayed(fun () ->
                    [ let prop =
                          ProvidedProperty(
                              "Indicators",
                              indicatorsDescriptionsType,
            ...
            let topicsType =
                let topicCollectionType =
                    ProvidedTypeBuilder.MakeGenericType(typedefof<TopicCollection<_>>, [ topicType ])

                let t =
                    ProvidedTypeDefinition(
                        "Topics",
                        Some topicCollectionType,
                        hideObjectMethods = true,
                        nonNullable = true
                    )
                ...
                t.AddMembersDelayed(fun () ->
                    [ for topic in connection.Topics do
                          ...
                          if not (String.IsNullOrEmpty topic.Description) then
                              prop.AddXmlDoc(topic.Description)
                ...
                t.AddMembersDelayed(fun () ->
                    [ ProvidedProperty(
                      ...
                      ProvidedProperty(
                          ...
                          topicsType,
                          getterCode =
                              (fun (Singleton arg) -> <@@ ((%%arg: WorldBankData) :> IWorldBankData).GetTopics() @@>)
                      ) ])
    ...
    let paramWorldBankType =
        let t =
            ProvidedTypeDefinition(asm, ns, "WorldBankDataProvider", None, hideObjectMethods = true, nonNullable = true)
