...
module internal Misc =
    ...
    let transExpr isGenerated q =     
        let rec trans q = 
            ...
            | Quotations.Patterns.PropertyGet(obj,propInfo,args) -> 
                match obj with 
                | None -> trans (Quotations.Expr.CallUnchecked(propInfo.GetGetMethod(),args))
                | Some o -> trans (Quotations.Expr.CallUnchecked(trans o,propInfo.GetGetMethod(),args))
            // Eliminate F# property sets to method calls
            | Quotations.Patterns.PropertySet(obj,propInfo,args,v) -> 
                 match obj with 
                 | None -> trans (Quotations.Expr.CallUnchecked(propInfo.GetSetMethod(),args@[v]))
                 | Some o -> trans (Quotations.Expr.CallUnchecked(trans o,propInfo.GetSetMethod(),args@[v]))
            ...
            | Quotations.Patterns.LetRecursive(bindings, expr) ->
                ...
                //   body
                vars
                |> List.fold (fun b v -> Quotations.Expr.LetUnchecked(varDict.[v], init v.Type, b)) body                
                |> trans 
    ...
    let transQuotationToCode isGenerated qexprf (paramNames: string[]) (argExprs: Quotations.Expr[]) = 
        ...
        let expr = Array.foldBack (fun (arg, var) e -> Quotations.Expr.LetUnchecked(var, arg, e)) pairs expr
        let expr = 
            if isGenerated then
    ...
    interface ITypeProvider with
        ...
        member __.GetInvokerExpression(methodBase, parameters) =
            ...
            and expand expr = 
                ...
                | ShapeVarUnchecked v -> Quotations.Expr.Var v
                | ShapeLambdaUnchecked (v, body) -> Quotations.Expr.Lambda(v, expand body)
