...
module internal Debug = 
    ...
    type DllInfo(path: string) = 
        member x.FileName = path

    type TcImports(bas: TcImports option, dllInfos: DllInfo list) =
       member x.Base = bas
    ...
    let generate (resolutionFolder: string) (runtimeAssembly: string) (runtimeAssemblyRefs: string list) typeProviderForNamespacesConstructor args =
    ...
    let inline private getAttrs attrName m = 
    ...
    let inline private hasAttr attrName m = 
        not (Seq.isEmpty (getAttrs attrName m))


    /// Append some text if a type comes from the design-time assembly
    let rec private warnIfWrongAssembly (t:Type) =
        match t with
        | :? ProvidedTypeDefinition -> ""
    ...
    let prettyPrint signatureOnly ignoreOutput maxDepth maxWidth (t: ProvidedTypeDefinition) = 
        ...
        let rec toString useFullName (t: Type) =
            ...
            let hasUnitOfMeasure = (t :? ProvidedSymbolType) && (t :?> ProvidedSymbolType).IsFSharpUnitAnnotated

            let innerToString (t: Type) =
                match t with
                | t when t.IsArray -> (t.GetElementType() |> toString useFullName) + "[]"
                | t when t.IsByRef -> (t.GetElementType() |> toString useFullName) + "&"
                | t when t.IsPointer -> (t.GetElementType() |> toString useFullName) + "*"
                | :? ProvidedTypeDefinition as t ->
                    add t
                    t.Name.Split(',').[0]
                | t when t.IsGenericType ->
                    let gtd = t.GetGenericTypeDefinition()
                    let fullName = getFullName gtd
                    let args =
                        if useFullName then
                            ...
                            t.GetGenericArguments() 
                            |> Seq.map (fun _ -> "_")
                    if fullName.StartsWith "System.Tuple`" then
                        String.concat " * " args
                    elif fullName.StartsWith "Microsoft.FSharp.Core.FSharpFunc`" then
                        "(" + (String.concat " -> " args) + ")"
                    else 
                        let args = String.concat "," args
                        let name, reverse = 
                            match t with
                            | _ when hasUnitOfMeasure -> toString useFullName t.UnderlyingSystemType, false
                            | _ when fullName = typedefof<int seq>.FullName -> "seq", true
                            | _ when fullName = typedefof<int list>.FullName -> "list", true
                            | _ when fullName = typedefof<int option>.FullName -> "option", true
                            | _ when fullName = typedefof<int ref>.FullName -> "ref", true
                            | _ when fullName = typedefof<Async<_>>.FullName -> "async", true
                            | _ when ns.Contains gtd.Namespace -> gtd.Name, false
                            | _ -> (if useFullName then fullName else gtd.Name), false
                        ...
                        if reverse then
                            ...
                            name + "<" + args + ">"
                | _  ->
                let fullName = getFullName t
                match t with 
                | _ when fullName = "System.Boolean" -> "bool"
                | _ when fullName = "System.Object" -> "obj"
                | _ when fullName = "System.Int32" -> "int"
                | _ when fullName = "System.Int64" -> "int64"
                | _ when fullName = "System.Double" -> "float"
                | _ when fullName = "System.Single"-> "float32"
                | _ when fullName = "System.Decimal" -> "decimal"
                | _ when fullName = "System.String" -> "string"
                | _ when fullName = "System.Void" -> "()"
                | _ when fullName = typeof<unit>.FullName -> "()"
                | t when ns.Contains t.Namespace -> t.Name
                | t when t.IsGenericParameter -> t.Name
                | t -> if useFullName then fullName else t.Name

            if ignoreOutput then
                ""
            elif hasUnitOfMeasure || t.IsGenericParameter || t.IsArray || t.IsPointer || t.IsByRef || t.DeclaringType = null then
        ...
        let printExpr expr =
            ...
            and printExpr fromPipe needsParens = function
                ...
                | Value (null, _) ->
                    print "null"
                | Value (value, typ) when typ.FullName = typeof<string>.FullName && (value :?> string).Contains("\\") ->
                    bprintf sb "@%A" value
                | Value (value, _) ->
                    bprintf sb "%A" value
                | Var (var) ->
