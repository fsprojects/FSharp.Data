...
module MimeTypes =
    ...
    let tryFind (ext: string) =
        Map.tryFind (ext.ToLowerInvariant()) map
...
module internal HttpHelpers =
                            ...
                            Some(
                                WebException(
                                    sprintf
                                        "%s\nResponse from %s:\n%s"
                                        exn.Message
                                        newResponse.ResponseUri.OriginalString
                                        responseText,
                                    exn,
                                    exn.Status,
                                    newResponse
    ...
    let setHeaders headers (req: HttpWebRequest) =
        ...
        headers
        |> List.iter (fun (header: string, value) ->
            ...
            | "date" ->
                req.Date <-
                    DateTime.SpecifyKind(
                        DateTime.ParseExact(value, "R", CultureInfo.InvariantCulture),
                        DateTimeKind.Utc
                    )
    ...
    let getResponse (req: HttpWebRequest) silentHttpErrors =
        ...
        let getResponseAsync (req: HttpWebRequest) =
            if req.Timeout = Timeout.Infinite then
...
module internal CookieHandling =
    ...
    let getAllCookiesFromHeader (header: string) (responseUri: Uri) =
        ...
        let isInvalidCookie (cookieStr: string) =
            ...
            equalsPos = -1
            || let semicolonPos = cookieStr.IndexOf ';' in
    ...
    let getCookiesAndManageCookieContainer
        uri
        responseUri
        (headers: Map<string, string>)
        ...
        | Some cookieHeader ->
            ...
            |> Array.fold
                (fun cookies (uri, cookie) ->
                    ...
                    cookies |> Map.add cookie.Name cookie.Value)
...
type Http private () =
    ...
    static member private InnerRequest
        ...
        | Some cookies ->
            cookies
            |> List.ofSeq
            |> List.iter (fun (name, value) -> cookieContainer.Add(req.RequestUri, Cookie(name, value)))
        ...
        augmentWebExceptionsWithDetails (fun () ->
            async {
                ...
                return!
                    toHttpResponse
                        resp.ResponseUri.OriginalString
                        statusCode
                        contentType
                        characterSet
                        responseEncodingOverride
