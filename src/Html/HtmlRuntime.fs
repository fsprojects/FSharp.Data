...
type HtmlDocument internal (doc, tables, lists, definitionLists) =
    ...
    static let create includeLayoutTables doc =
        let tables = 
            doc
    ...
    static member Create(includeLayoutTables, reader:TextReader) =
        let doc = 
            reader 
            |> HtmlDocument.Load
        create includeLayoutTables doc

    /// [omit]
    [<EditorBrowsableAttribute(EditorBrowsableState.Never)>]
    [<CompilerMessageAttribute("This method is intended for use in generated code only.", 10001, IsHidden=true, IsError=false)>]
    static member AsyncCreate(includeLayoutTables, reader:TextReader) =
        async {
            let! doc = 
                reader 
                |> HtmlDocument.AsyncLoad
            return create includeLayoutTables doc
        }
