...
type HtmlColumn internal (values:string[]) = 
    
     member internal x.GetValue(propertyIndex:int) = 
        values.[propertyIndex]

/// Underlying representation of table types generated by HtmlProvider
type HtmlRow internal (headers:string[], columns:HtmlColumn[]) =
    
    member x.Headers = headers
    member x.Columns = columns

    member internal x.GetColumn(columnIndex:int) = 
       columns.[columnIndex]
    ...
    static member Create(doc:HtmlDocument, id:string, hasHeaders:bool) =
        ...
        | Table table -> 
            ...
            let rows = rows |> Array.map (fun r -> new HtmlRow(headers, r |> Array.map (fun x -> new HtmlColumn(x.AsStringArray()))))
            rows
