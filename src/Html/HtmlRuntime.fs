...
type HtmlRuntime =
    ...
    static member TryGetValue(n:HtmlNode) = 
      if String.IsNullOrEmpty(n.InnerText()) then None else Some (n.InnerText())
    
    static member TryGetAttribute(n:HtmlNode, name) = 
      n.TryGetAttribute(name) |> Option.map (fun a -> a.Value) 
    ...
    static member private GetChildrenArray(n:HtmlNode, name:string) =
      n.Elements(name) |> List.toArray
    
    static member private GetChildOption(value:HtmlNode, name) =
    ...
    static member GetChild(value:HtmlNode, name) =
      match HtmlRuntime.GetChildrenArray(value, name) with
      | [| it |] -> it
    ...
    static member ConvertArray<'R>(n:HtmlNode, name, f:Func<HtmlNode,'R>) : 'R[] = 
      HtmlRuntime.GetChildrenArray(n, name) |> Array.map f.Invoke
    
    static member ConvertOptional<'R>(n:HtmlNode, name, f:Func<HtmlNode,'R>) =
      HtmlRuntime.GetChildOption(n, name) |> Option.map f.Invoke
    
    static member ConvertOptional2<'R>(n:HtmlNode, name, f:Func<HtmlNode,'R option>) =
    ...
    static member ConvertAsName<'R>(n:HtmlNode, name, f:Func<HtmlNode,'R>) = 
      if n.Name = name then Some(f.Invoke n)
      else None
...
type HtmlRuntimeWrapper() = 
    ...
    static member Create(doc:HtmlDocument, id:string, hasHeaders:bool, headers:string[]) =
        (doc.GetObject id).ToHtmlElement(hasHeaders, headers)
